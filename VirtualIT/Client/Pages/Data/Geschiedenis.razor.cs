...
namespace VirtualIT.Client.Pages.Data
{
    public partial class Geschiedenis
    {
        ...
        {
            string url = "api/VirtualMachine/GetVerlopenWithFilters";
            var query = new QueryDictionary<StringValues>();
            query.Add("grid-page", "1");
            var client = new GridClient<VirtualMachineDto.Detail>(HttpClient, url, query, false, "ordersGrid", Columns).Sortable().Filterable().Searchable(true, false, true).ClearFiltersButton(true).SetStriped(true).WithMultipleFilters();
            _grid = client.Grid;
